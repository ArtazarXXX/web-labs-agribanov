...
new Vue({
  ...
  data: {
    ...
    userNameBoolevo: false, ///// true на проде поставить !!!
  ...
  methods: {
    ...
    checkTextInInput: function(search){
      if(search != '')
        ...
        this.uuid++
        this.plan_array.push(objects('Задача ' + this.uuid, search, "", "", this.userName, "plan"))
        this.search = search
        this.status = ''
        this.sponsor = ''
        this.start = ''
    ...
    add_data_to_array: function(desc, status, sponsor, start, end){
      if((status == 'plan' || status == 'in-work' || status == 'ended') && this.editable == false){
          this.uuid++
          this.plan_array.push(objects('Задача ' + this.uuid, desc, start, end, this.userName, status, ""))
          this.reset_input()
          this.windowVisibility = !this.windowVisibility
          this.shell()
      }
                ...
                for(let i = 0; i < this.plan_array.length; i++){
                  if(this.plan_array[i]["title"].includes(this.selected_id)){
    ...
    edit_app_block: function(index){
      for(let i = 0; i < this.plan_array.length; i++){
        if(this.plan_array[i]["title"].includes(index + 1)){
      ...
      for(let i = 0; i < this.plan_array.length; i++){
        if(this.plan_array[i]["title"].includes(index + 1)){
          for(let j = i + 1; j < this.plan_array.length; j++){
            this.plan_array[j]["title"] = 'Задача ' + (this.plan_array[j]["title"].split(" ")[1] - 1)
          }
    ...
    onDrop: function (evt, list) {
      ...
      const item = this.plan_array.find(item => item.title == itemID)
      item.tag = list
